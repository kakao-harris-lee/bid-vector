...
class InferenceOutboxSettings(BaseSettings):
    ...
    #   models/sbert/...      service  코퍼스 3,593 · 임계 17.97행 · 활성 2,790 · +695/일
    #   (라이브)              construction 코퍼스 2,811 · 임계 14.06행 · 활성 1,812 · +596/일
    #   /app/models/sbert/... 코퍼스 68,827 · 활성 1,220 · **2026-08-04 이후 write 0**
    #   활성 합계 5,822
    #
    # 라이브 모델에서는 0.5% 임계가 18행/14행에 불과해 최대 수명(6h)보다 **훨씬 먼저**
    # 구속한다: 17.97행 ÷ 28.96행/시 ≈ **37분**마다 다시 material 이 된다. 즉 무효화
    # 동인은 수명이 아니라 drift 다.
    #
    ...
    # 넘긴 뒤 선택 지연 안에 해소" — 그 지연이 분 단위로 묶인다는 뜻이다).
    #
    ...
    #
    # 보장되지 **않는** 것: 0.5% drift 임계. 라이브 모델 대상은 37분마다 임계를 넘는데
    # 재방문은 ~2.6시간마다이므로, 서빙되는 스냅샷이 임계의 최대 ~4배(코퍼스의 약 2%)
    # 까지 밀릴 수 있다. drift 까지 상한으로 만들려면 회전 주기를 37분 이하로 줄여야
    # 하고 그건 4,602/0.62h ≈ 7,400건/시로 **소진 처리량 6,000건/시를 넘는다**. 지금
    # 파이프라인 안에서는 불가능하므로 시도하지 않는다 — 여기서 보장하는 것은 수명뿐이다.
    #
