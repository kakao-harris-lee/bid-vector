...
# ── 유사공고 투영(projection) 신선도 정책 ────────────────────────────────────
# 배경: 이 두 상수가 생기기 전, 백필은 스냅샷의 코퍼스 워터마크가 라이브 워터마크와
# **정확히 일치**할 때만 신선하다고 봤다. 워터마크 스코프는 (embedding_model,
# category) 이므로 그 카테고리에 임베딩이 **1건**만 들어와도 그 카테고리의 활성
# 대상 전부가 동시에 stale 이 된다. 한 회전(활성 5,708건)에 걸리는 시간보다 코퍼스가
# 훨씬 자주 움직여서 후보 집합이 구조적으로 드레인되지 않았고, 백필이 4시간 동안
# 193회 연속 실행되며 inference 워커를 점유했다(2026-08-13 사고. 실측: 후보 5,432건
# 중 no_snapshot 0 · edge_mismatch 0 — 전부 워터마크 신선도만으로 후보가 됐고,
# 그중 시각만 어긋난 건은 0건이라 count 항이 유일한 구속 조건이었다).
#
# 정책: "스냅샷이 요약한 코퍼스가 **의미 있게** 달라졌을 때만 다시 계산한다.
# 그렇지 않으면 최대 수명까지는 그대로 쓴다."
#
# 스냅샷은 코퍼스 N 건 중 상위 20건(top-k)을 담는다. 새 k 건이 기존 20위를 밀어낼
# 확률은 신규 행을 같은 분포의 교환가능한 추출로 보면 대략 20k/N 이다. k = 0.005N
# 이면 그 확률이 약 10% — "스냅샷의 상위 20 중 한 자리가 바뀌었을 만한" 경계다.
# 이 미만의 변화는 스냅샷이 여전히 코퍼스를 기술한다고 보고 재계산하지 않는다.
# 비율이 1행 미만으로 떨어지는 작은 코퍼스(N<200)에서는 20/N 이 커서 한 건의 추가도
# 상위 20을 흔들므로, 판정은 `max(1행, 비율)` 로 바닥을 둔다(0 건 변화는 언제나 무관).
SIMILARITY_PROJECTION_CORPUS_DRIFT_MATERIALITY_RATIO: float = 0.005

# 물질성만으로는 두 가지가 새므로 최대 수명을 함께 둔다: ①조용한 코퍼스에서는
# 스냅샷이 무한히 늙는다 ②기존 행의 **재임베딩**은 건수를 바꾸지 않아 물질성 판정에
# 잡히지 않는다. 즉 이 값이 "투영이 코퍼스와 어긋나 있을 수 있는 최대 시간"의 단일
# 상한이다.
#
# 6시간인 이유: 이 투영은 며칠씩 열려 있는 공고의 의사결정 보조 읽기라, 반나절 이내
# 신선도면 판단이 달라지지 않는다. 부하는 활성 대상 수 / 수명 = 5,708/6h ≈ 951건/시,
# 투영 1건의 pgvector 상위 20 검색 실측이 p50 27ms(n=25, 최대 43ms)이므로 시간당
# 워커 점유가 1분 미만(<2% duty cycle)이다. 더 짧게 잡아도 부하는 감당되지만 얻는
# 신선도가 의사결정을 바꾸지 않고, 더 길게 잡으면 재임베딩 반영이 눈에 띄게 늦는다.
SIMILARITY_PROJECTION_MAX_SNAPSHOT_AGE_SECONDS: int = 6 * 60 * 60
