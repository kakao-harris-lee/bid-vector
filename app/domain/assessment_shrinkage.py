...
왜 계층 수축인가
----------------
발주기관별 사정률 점추정은 성립하지 않는다: 운영 DB 실측으로 발주기관 5,309곳 중
표본 ≥150 은 단 2곳, ≥30 은 204곳, **<10 이 4,440곳(84%)** 이다. 표본이 얕은
기관을 자기 평균으로 점추정하면 노이즈를 그대로 추천에 싣는다. 그래서 관측 평균을
표본 수의 함수로 상위 계층(공종, 최종적으로 전역)으로 **수축(shrinkage)** 한다.
...
#
# * 기관 κ=12: method-of-moments κ=σ²_within/σ²_between 은 추정기에 민감하다 —
#   관측 그룹평균 분산을 그대로 쓰면(naive) ≈8, 표본 노이즈를 빼면(corrected)
#   ≈19~∞(n≥2 코호트에서는 노이즈 제거 후 기관 간 신호가 검출되지 않음), ANOVA
#   MoM ≈36. 즉 데이터는 "기관 간 신호가 약하다 = 강한 수축"을 지지하고, 12 는
#   naive(8)와 corrected(19) 사이의 보수적 중간값이다. 점추정이 아니라 커버리지
#   (PIT) 축이 이 선택을 end-to-end 로 검증한다. Phase 2 에서 릴리스 단위로
#   재추정한다.
...
def resolve_assessment_posterior(
    *,
    ...
    계층이 없거나 표본이 0이면 그 계층의 가중치는 정확히 0이 되어 상위 계층으로
    투명하게 떨어진다(84% 얕은 기관이 바로 이 경로다). 세 가중치의 합은 1이다.
    """
