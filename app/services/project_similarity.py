...
class ProjectSimilarityService(ProjectEmbeddingBatchMixin):
    ...
    ) -> Callable[[Project], tuple[list[float], str | None]]:
        if read_only:
            return self.resolve_embedding_without_persist
        return self._stored_embedding_with_model

    def _stored_embedding_with_model(
        self, project: Project
    ) -> tuple[list[float], str | None]:
        """Read a candidate's stored vector with the model column that produced it.

        모델이 비어 있는 저장본에는 이름을 지어내지 않고 ``None`` 을 그대로
        돌려준다 — 호출부의 모델 필터가 걸러내므로, NULL 을 절대 매칭하지 않는
        pgvector 의 SQL 등치(:meth:`_search_with_postgres`)와 같은 판정이 된다.
        """
        return self._load_embedding(project), project.embedding_model
    ...
    def _search_with_python(
        ...
        *,
        ...
        limit: int,
        min_similarity: float,
        embedding_resolver: Callable[[Project], tuple[list[float], str | None]]
        | None = None,
    ) -> list[dict[str, Any]]:
        """Fallback similarity search using stored embedding payloads.

        ``embedding_resolver`` 은 candidate → (벡터, 그 벡터를 만든 모델) 조회를
        갈아끼우는 seam 이다. 기본값(``None``)은 저장 컬럼을 그대로 읽는
        :meth:`_stored_embedding_with_model` 이고, read-only 스캔은 세션 쓰기 없이
        재해석하는 resolver 를 주입한다.

        불변식은 "다른 좌표계의 벡터를 채점하지 않는다"이다. 다른 모델로 만들어진
        벡터는 코사인 값이 의미를 잃지만 :meth:`_cosine_similarity` 는 zip 이라
        조용히 점수를 내므로, 채점에 실제 쓰는 벡터의 모델이 질의 모델과 다르면
        (NULL 포함) 후보를 건너뛴다. pgvector 경로는 같은 불변식을 저장 컬럼
        등치로 지킨다. 두 경로의 후보 집합은 대부분 일치하지만 완전히 같지는
        않다: read-only 재해석 resolver 는 저장 컬럼이 낡았어도 질의 모델로 다시
        계산한 벡터를 낼 수 있어, 그 경우에 한해 pgvector 보다 넓은 유효 후보를
        본다(어느 경우에도 교차 좌표계 채점은 없다).
        """
