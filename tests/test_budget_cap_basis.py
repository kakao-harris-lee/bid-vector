...
된다(홀드아웃 실측: 그 구간 34건 중 28건이 낙찰가에서 멀어짐). 그래서 상한을 넘길
권한은 guardrail 이 낸 ``floor_price``(법정 하한을 max() 로 접어 넣은 하한 그 자체)가
보고된 경우로 한정한다.

실투찰 3건은 실격 1건(하한·사정률 축) + 패찰 2건(과추천 축)이다. 이 수정은 **실격과 같은
계열(하한 미달)의 별개 결함**을 제거하며, 그 사건의 원인은 아니다 — 8/3 공고는
기초금액/추정가격 비가 1.100 이라 이 clamp 의 발동 조건(비 > 1.136)에 들지 않고, 제출가
자체가 이 함수를 통과하지 않았다(수기 제출). 실격 원인은 예정가 추첨 +0.558% 로 확정됐다.
"""
