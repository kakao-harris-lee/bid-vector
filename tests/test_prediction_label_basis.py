...
같은 PR 이 위임한 ``floor_shortfall.assessment_rate_from_opening`` 의 특성화 값표는
``tests/test_assessment_rate_characterization.py`` 에 따로 있다 — 주제가 다르고(그쪽은
운영자 대면 하한 미달 빈도의 표본 선택), 합쳐 두면 파일이 §4.5-4 한도(~500줄)를 넘긴다.
"""
