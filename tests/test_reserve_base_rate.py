...
def test_summarize_survives_out_of_band_bid_rate_with_reserve_pattern():
    """회귀: 밴드 밖 bid_rate(None 해석) + 예비가·추첨번호 행에서 죽지 않아야 한다.

    ``resolve_record_bid_rate`` 는 0.5~1.5 밴드 밖(percent-스케일 오적재 87.5 등)을
    ``None`` 으로 돌려준다. 예비가격 패턴 블록이 그 ``None`` 을 float 밴드와 비교해
    ``TypeError`` 로 죽던 실데이터 크래시의 재현이다(코어 clean 행에서 실측).
    """
    reserve_prices = [99_000_000.0 + (index * 200_000.0) for index in range(15)]
    out_of_band = {
        "bid_rate": 87.5,  # percent-스케일 오적재 → 사용 불가(None) 판정 대상
        "base_amount": 100_000_000.0,
        "reserve_prices": reserve_prices,
