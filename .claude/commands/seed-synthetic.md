---
description: synthetic 운영자 시드 (scripts/seed_synthetic_operators.py)
argument-hint: "[--purge]"
allowed-tools:
  - Bash
  - Read
...
- 운영 DB URL로 실행
- 시드 이후 자동 백테스트 실행 (사용자가 명시적으로 `/run-backtest` 할 때만)

## 보고

- 실행한 명령
- 생성/갱신/삭제된 row 수 (스크립트 stdout에서 추출)
- 다음 권고: `/run-backtest`로 결과 비교 가능
