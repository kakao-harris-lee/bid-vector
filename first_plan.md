제공해주신 상세 리포트의 기술적 분석과 비즈니스 로직을 바탕으로, AI Agent(Cursor, GitHub Copilot, AutoGPT 등)가 바로 코딩에 착수할 수 있도록 수준 높은 **[AI Agentic Coding을 위한 개발 기획 지시서]**를 작성해 드립니다.
이 지시서는 맥(Mac) 환경에서 파이썬(Python)을 기반으로 하며, 리포트에서 언급된 '사정률 예측' 및 '공정 분배 로직' 등 고도화된 기능을 포함합니다.
...
• 핵심 가치: 낙찰 확률 극대화, 데이터 기반 사정률 예측, 사용자 간 공정 배분.
