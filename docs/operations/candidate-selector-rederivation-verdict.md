...
3. **모집단 반전 문제**: post-#312 재실행에서 floor_bound clean 표본은 642건으로 게이트를 명목상 통과했지만(스코프 주의: 1차 분석의 67건은 07-27 홀드아웃, 이번 642건·service 80건은 07-28 재생성 6,121건 홀드아웃의 수치 — 서로 다른 스냅샷이라 67→80을 #312 효과로 읽으면 안 된다), 그 다수는 재라벨된 공사 수의 행이고 해당 셀의 역사적 최적 스탠스는 base였다. 라벨 재정의가 Option A의 전제(당시 floor_bound ≈ 용역)를 바꿨으므로, **새 라벨 분포 기준으로 스탠스 최적성·정책 표를 재도출한 뒤에만 배선을 판단**하기로 했다. 본 문서가 그 재도출의 판정 기록이다.
