...
| `separate_regime` | 국가기관이지만 별도 행정규칙 체계를 따르는 발주(산림청 계열) | 생략 |
...
**대체 하한율은 기록하지 않는다.** 그 행정규칙의 정확한 하한율과 개정 시점은 원문 미확인이라
`separate_regime`은 "국가계약 era-tier 비적용"이라는 사실만 남긴다(정직 명세 §2). 해석된
era-tier 값과 출처는 추적을 위해 상세에 그대로 유지하되 판정에는 쓰지 않는다. 비국가기관
(`not_applicable`)과 **같은 버킷으로 접지 않는다** — 산림청은 국가기관이 맞고 근거가 다르다.
...
기관 축 판별이라 **사업 종류 축은 구분하지 못한다.** 산림청 소속기관이 발주한 일반
공사·용역까지 함께 생략되므로, 이 상태는 "하회 아님"이 아니라 **판정 보류**다.
...
`below_legal_floor`로 남긴다. `separate_regime`도 "국가계약 tier 비적용"까지만 말하고 대체 하한을
주장하지 않으므로, 산림사업 하회 여부는 **판정 보류** 상태다(예규 원문 확인 시 별도 tier 테이블로
승격 가능). 이 판별은 전부 분석 전용이며 라이브 예측 guardrail(게시 하한을 `max()`로만 접어
올리는 경로)에는 관여하지 않는다.
