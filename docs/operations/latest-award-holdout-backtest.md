...
| `separate_regime` | 국가기관이지만 별도 행정규칙 체계를 따르는 발주(산림청 계열) | 공사만 수행(산림사업 하한 87.745%), 용역·물품은 생략 |
...
추종하지 않았다. 그래서 `separate_regime`의 **공사** 공고는 이제 era-tier 대신 **산림사업 하한
87.745%로 하회 판정을 수행**하고(`legal_floor_source = forestry_regime_spec`), 상수는
...
**적용은 공사 카테고리 한정이다.** 확인된 원문(산림토목)도 라이브 표본 6건도 전부 공사
적격심사 하한이라, era-tier와 같은 공사 게이트를 통과한 행에만 이 하한을 댄다. 산림청이 발주한
용역·물품은 어느 하한 체계인지 근거가 없고 용역 적심 하한은 공사보다 낮으므로, 하한을 해석하지
않고(`legal_floor_source = unresolved`) 판정을 생략한다 — 공사 하한을 그대로 댔다면 적법한 용역
낙찰이 하회로 오탐됐을 자리다.

그래도 기관 축 판별이라 **공사 안에서 사업 종류는 구분하지 못한다.** 산림청 소속기관이 발주한
일반 공사(산림사업이 아닌 건축·토목 등)까지 같은 하한으로 재는데, 그쪽이 실제로는 국가계약
적격심사(0.89745) 대상이라면 더 낮은 하한을 댄 셈이라 **하회를 놓칠 수는 있어도 없는 위법을
만들지는 않는다**(오탐이 아니라 미검출 방향).
