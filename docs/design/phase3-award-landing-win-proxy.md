...
**α 제약의 방향(PR1 구현 확인)**: `P(φ>b)` 는 `b` 에 **비증가**이므로 실행가능 집합은 상향폐집합 `[b_min(α), ∞)` 이고, α-제약은 착지점을 **위로** 민다. "보수적 = 낮은 가격"이라는 통상 직관과 **반대**다. **이 방향 진술은 실데이터에서도 유효하다.**

**정정(PR2 실측, 2026-08-14) — "제약이 걸리면 통상 `b* = b_min(α)`, 실측 93%"는 실데이터에서 재현되지 않는다.** 그 93% 는 PR1 **커널 검증의 합성 fixture** 실측이었고 실코퍼스 측정이 아니었다. PR2 가 라이브 코퍼스에서 재산출한 binding 비율은 **G3 통과 셀 기준 5.0%**(대조군 게이트 `not_negotiated`, 4셀) / **0.0%**(설계 충실 게이트 `floor_bound`, 2셀)다. 원인은 실코퍼스 `WW` 의 모양이다: 층 A 는 `1/N` 계단의 **다봉** 곡선이고(최심 셀 `mode_count` = 194), 하한 부근에서는 `b` 를 올릴 때 하한을 넘기는 개찰이 늘어 생존항이 커지는 이득이 `ω ≥ b` 를 잃는 손실보다 잠깐 크다. 그래서 α-제약 argmax 가 `b_min(α)` 로 붕괴하지 않고 그 **위 국소 봉우리**에 앉는다 — `b* − b_min(α)` 중앙값은 **G3 통과 셀 기준 대조군 5.95bp / 설계 충실 게이트 3.15bp**다.

> **인용 시 스코프를 밝힐 것**(N1): 전-셀 집계는 판정 불가(얕은) 셀을 같은 평균에 섞어 값을 희석한다 — 같은 거리 중앙값이 전-셀에서는 12.19bp / 12.80bp 로 두 배 넘게 커지고, 이득 중앙값은 대조군에서 **0.0 으로 붕괴**한다. PR2 리포트는 `constrained_deep_cells` 와 `constrained_all_cells` 를 **이름으로 구별해 병기**하며, 판정에 인용해야 하는 것은 전자다.

**그 거리를 "사실상 binding"으로 읽으면 안 된다**(리뷰 정정): 거리가 0 이 아니라는 것은 실행가능 구간 **안에서 곡선이 실제로 골랐다**는 뜻이다. 그 선택이 신호인지는 `WW(b*) − WW(b_min(α))` 이득이 셀별 `1/N` 과 어디에 서는가로 갈리고, **결론의 강도가 게이트별로 다르다**: 대조군은 이득 중앙값 **8.29e-4 < 네 깊은 셀의 최소 `1/N`(1.66e-3)** 이라 "좌표 이동은 표본 잡음"이 유지되지만, 설계 충실 게이트는 **3.40e-3 ≈ `1/N` 의 1~2배**(1/294 와 정확히 같고 1/580 의 1.97배)라 **잡음이라고 단정할 수 없다**. PR4 배선 판정에서 bp 단위 `b*` 를 인용하기 전에 `mode_count`·`plateau_width`·이 이득을 **셀별 `1/N` 과 함께** 봐야 한다(§11-7).

> **provenance 주의**: 위 M2 의 실격률(공사 32.5% · 용역 52.6%)과 §6 M5 의 f 분포·§NEW-4 의 상관 인용은 전부 **floor_bound 레짐 게이트를 적용하지 않은** 코퍼스에서 산출됐다(§4 셀 깊이표가 "하드 필터 전 상한"이라고 헤지한 것과 같은 모집단이다 — 그쪽만 헤지가 있었다). 게이트를 적용한 PR2 기본 코퍼스에서는 값이 갈리는 것이 정상이며, 두 수치를 나란히 놓는 대조표가 `app/services/ml_training/award_landing_diagnostics.py` 의 `QUOTED_REFERENCES` 다. 실측 예: 용역 실격률은 게이트 미적용에서 51.6%(≈인용값)지만 게이트 적용에서는 17.9% 로 갈린다.
