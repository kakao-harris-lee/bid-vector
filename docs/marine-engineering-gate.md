...
| 축 | 값 | 비고 |
|---|---|---|
| 카테고리 | `business_type = technical-service`, `focus_categories = technical-service, service` | 설계·감리·조사·측량 등 기술용역 |
...
- `해양엔지니어링`은 의도적으로 **ENG001 + MAR001 둘 다** 매칭된다 — `엔지니어링`이
  ...
  하므로 의도된 동작이다.
- `…기술사`(항만및해안기술사·해안기술사·해양기술사)는 기존 `기술사` 별칭으로
  **ENG001이 함께** 잡힌다. 같은 엔지니어링 계열 신호이므로 의도된 동작이다.
...
구성한다. 면허제한 미수집 공고도 제목/본문 키워드로 식별한다.

키워드 38개(항만/해양/연안 토목·기술용역 어휘):

```
항만 어항 방파제 안벽 부두 잔교 선착장 물양장 호안 방조제 갑문 계류 케이슨
준설 매립 사석 해상 해양 해저 수중 연안 해안 갯벌 조간대 해양조사 수로측량
조위 조류관측 해양환경 항만설계 해안침식 항로 항로표지 등대 어초 인공어초
마리나 수산
```
