...
2026-07-18 기준으로 Phase 3 초기 후속이 반영되었습니다 — 공사 법정 낙찰하한율 개정 반영 완주(#197~#200, 공사 추천 오차 0.305% 실증), 실투찰 검증 파이프라인 완비(#201~#203: 낙찰하한율 자동 수집·검증 폴백·backfill, 투찰가 정수 정합). 2026-07-25 기준으로 실투찰 2건은 낙찰자 확정(2026-07-20 수집, 두 건 모두 패찰=적격+밀림)에 따라 **#195 발주처 밴드 가설 재검증이 완료**됐습니다 — 현 predictor는 확정 낙찰가 대비 +0.34%p/+0.68%p 과추천, 원인은 floor 앵커 base 불일치(법정 하한 0.88을 예정가 아닌 기초금액에 곱함)로 규명, n=2 과적합 방지로 재캘리브레이션 보류(`docs/operations/real-bid-195-revalidation.md`). 2026-07-27 기준으로 **floor 앵커 base 정합 가설 검증이 완료됐습니다 — 불지지·앵커 하향 안 함**(floor-bound 실측 코어 532쌍: 실측 사정률 평균 0.9974·37.6%는 >1, E[사정률] 전환은 잔차 무개선·실격측 52%로 악화 — 상세 `docs/operations/floor-anchor-recalibration-verdict.md`). 남는 추천 정밀도 축은 앵커가 아니라 recommended candidate selector(하한 위 착지점 선택 — "다음 우선순위" 5번)입니다. 2026-07-28 기준으로 **selector 정책 재도출도 완료됐습니다 — 전체 셀 승격(구 Option A)은 기각, service 한정 후보(P1')만 draft로 생존하나 표본 게이트 미충족(n=80<150)이라 배선 보류**(상세 `docs/operations/candidate-selector-rederivation-verdict.md`). 현재 병목은 빌드가 아니라 엔지니어링협회 실증 코어 잔여 소진과 service floor_bound 표본 축적입니다(코어 A→B→C 자체는 #206~#213으로 구축·배포 완료 — 아래 "다음 우선순위" 2번 참고).
...
2026-07-25로 다음이 반영되었습니다(문서 정정).

- 실투찰 2건 #195 발주처 밴드 가설 **재검증 완료** — 낙찰자 확정(2026-07-20, 두 건 패찰=적격+밀림) 기준. 현 predictor 추천이 확정 낙찰가 대비 +0.34%p/+0.68%p 과추천, 원인은 floor 앵커 base 불일치(법정 하한 0.88을 예정가 아닌 기초금액에 곱함·사정률<1 구조적 초과)로 규명. n=2 과적합 방지로 재캘리브레이션 보류·다음 가설 고정. predictor 코드 무변경. 리포트: `docs/operations/real-bid-195-revalidation.md`.
...
- **P0 완료 (#195)**: 발주처 밴드를 E[사정률](예정가/기초금액, 발주처별 실측 평균) 변환으로 사업금액 기준에 정합화 + scsbid가 예정가를 `base_amount`로 폴백 저장하던 오염 경로 제거. guardrail red line(카테고리/법정 하한 `max()` 구조) 불변 — ml-reviewer 확인. **재검증 완료(2026-07-25, `docs/operations/real-bid-195-revalidation.md`)**: 두 공고 확정 낙찰가로 검증한 결과 E[사정률] 변환이 발주처 밴드 edge에만 적용되고 하드 법정 floor(예정가-basis 0.88을 기초금액에 곱함)엔 미적용이라 `max()`가 보정을 상쇄, 현 추천이 낙찰가 대비 +0.34%p/+0.68%p 과추천으로 남음. n=2 과적합 방지로 밴드/floor 재캘리브레이션은 보류, 다음 가설(floor 앵커 base 정합 = 0.88×E[예정가]) 고정.
...
- **잔여 후속(우선순위 순)**: ① #195 발주처 밴드 가설 재검증 **완료(2026-07-25)** — 두 공고 모두 패찰(적격+밀림) 확정. 현 predictor는 확정 낙찰가 대비 +0.34%p/+0.68%p 과추천이며, 원인은 floor 앵커 base 불일치(법정 하한 0.88을 예정가 아닌 기초금액에 곱함, 사정률<1 구조적 초과)로 규명. #195 E[사정률] 변환은 발주처 밴드 edge에만 적용되고 하드 법정 floor엔 미적용→`max()`가 보정 상쇄(비대칭 확인). n=2 과적합 방지로 재캘리브레이션 보류·다음 가설(0.88×E[예정가]) 고정(→ **2026-07-27 검증 완료·불지지**, `docs/operations/floor-anchor-recalibration-verdict.md`). 상세 `docs/operations/real-bid-195-revalidation.md`. ② LLM 하이브리드는 **재검토 후 defer 확정(2026-07-24)** — 수치 예측은 통계/ML 유지(tabular 회귀 대비 열세·비결정성·감사불가). 원래 스코프 3개 중 ⓐ 공고문 자격조건 feature 추출·ⓑ 레짐 분류는 **결정적·감사가능 구현으로 이미 대체됨**(자격=구조화 `eligibility_raw.license_limits`+룰 해석기 #206~#213/#237/#238, 레짐=키워드 룰 `_detect_price_regime_signals` #239/#240) — 이 경로에 LLM 투입은 하드 게이트/가격 감사성에 환각·비결정 회귀라 **스코프에서 제외**. ⓒ 특약 리스크 요약(자문 전용·비스코어링)만 조건부 후속으로 남기되, 착수 시 수치예측·하드게이트·probability_score 미투입·LLM생성 명시·사람검토·prompt/response 감사로깅 전제, G-3 실수요 확인 후. 현재 seam(`app/ai/llm_interfaces.py`·`ExecutorDocumentAnalysisPort`)은 deferred 유지. ③ 개찰 참가업체별 순위/투찰금액 수집은 **종결(2026-07-19 실측)** — KONEPS OpenAPI 개찰결과목록(`getOpengResultListInfoServc`)이 1위+참가자수만 제공(표적조회 불가)하고 운영자가 전체 참가업체 분석 확장을 반려해, 개찰 1위(잠정) 수집(#211/#212)으로 대체 종결.
...
5. 추천 품질 세그먼트 후속: **핵심 대부분 완료(#239/#240)** — `price_regime_features`(`_build_price_regime_features`), `floor_bound`/`near_100`/`deep_discount`/`ambiguous` 레짐 라벨, recommended selector 분리(`_select_recommended_candidate`), 최신 N건 rolling holdout(`backtest_latest_award_holdouts.py`) 구현·배포. **기관 group holdout·분모/법정하한 품질 검사 세분화는 분석 경로에 구현**(`--group-by agency`, `--exclude-agency-history`, `--min-agency-samples`, `amount_rate_mismatch`/`below_legal_floor`/`base_basis_contaminated` 플래그, clean/flag 분리 집계, 고정 리포트 경로 `models/reports/latest-award-holdout-agency.json` — 절차는 `docs/operations/latest-award-holdout-backtest.md`). 기준선 측정(2026-07-26, below_legal_floor 오탐 49→0 정화 + 적용 범위·rate 독립성·산림 regime 게이트 #274/#288/#296)과 **floor 앵커 재캘리브레이션 판단(2026-07-27 검증 완료 — 불지지·앵커 하향 안 함, `docs/operations/floor-anchor-recalibration-verdict.md`)** 도 완료. **recommended candidate selector 정책 재도출 완료(2026-07-28, `docs/operations/candidate-selector-rederivation-verdict.md`)** — #312 라벨 재정의 후 post-#312 홀드아웃(6,121건)으로 표 동결→rolling 채점 재평가: 구 Option A(floor_bound 전체→conservative)는 OOS 악화·실격측 팽창으로 **기각**, 유일 생존 후보는 P1'(floor_bound×service 한정→conservative, 공사는 base 유지)이나 **해당 부분셀 표본 n=80<150으로 배선 보류(draft)**. 규칙: 이후 floor_bound 정책은 반드시 group-scoped. 잔여=원래-service floor_bound 표본 축적 후 P1' 재론(현행 always-base 유지, 코드 무변경). **후속 설계 확정(2026-08-13)**: 이 selector 트랙의 원리적 후계로 ML 3축 재편의 Phase 3(낙찰자 착지 분포 win-proxy 곡선)가 설계 승인됨 — 이산 3점 스탠스 승격 대신 반사실 삽입 `WW(b)` 연속 곡선으로 하한 위 착지점을 고른다(1차 = 옵션 A 곡선만·winner-only·배선은 G1~G8 게이트 통과 후, 상세 `docs/design/phase3-award-landing-win-proxy.md`).
6. 개찰 데이터 폭 확장: 참가업체별 순위/투찰금액 수집은 **종결**(OpenAPI가 1위+참가자수만 제공하는 실측 + 운영자 스코프 반려 — 개찰 1위 수집 #211/#212로 대체, Phase 3 잔여 후속 ③ 참고). 잔여는 기존 공고 낙찰하한율 backfill 소진(일일 쿼터 분할로 재개·진행 중).
...
- `docs/design/phase3-award-landing-win-proxy.md`: ML 3축 재편 Phase 3 설계(2026-08-13 승인) — winner-only 반사실 삽입 win-proxy 곡선, Phase 1 완전 열거 CDF 합성, 승격 게이트 G1~G8, PR 슬라이스
